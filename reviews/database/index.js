...
const users = function makeUsers(namesArray) {
  namesArray.forEach((name) => {
    ...
    connection.query('INSERT INTO users (name, userLoc, numFriends, numPhotos, numReviews, photoLoc) VALUES (?, ?, ?, ?, ?, ?)', [name, 'San Francisco, Ca', numFriends, numPhotos, numReviews, imgLoc]);
  });
};

const makeLocs = function generateLocations(restaurants) {
  restaurants.forEach((resto) => {
    connection.query('INSERT INTO locations (locname) VALUES (?)', resto, (err) => {
      if (err) {
        throw err;
      }
    });
  ...
  restaurants.forEach((resto, locIdx) => (
    reviewers.forEach((reviewer, userIdx) => {
      ...
      if (randomNum > 0.4) {
        ...
        connection.query('INSERT INTO reviews (message, stars, posted, userID, locID) VALUES (?, ?, ?, ?, ?)', [paragraph, stars, date, userID, locIdx + 1], (err) => {
          if (err) {
            throw err;
          }
        });
